...
public class ObisReader {
     ...
     * @return Les données lues sur Obis.
     */
    public static SpecieFeature loadOccurrencesFile(
            String name, int geoHashPrecision, String startDate, String endDate
    ) {
        try {
    ...
    public static ArrayList<ObservationDetails> loadObservationDetails(String geoHash, String name) {
       ...
       /*
    public static ArrayList<SpecieFeature> loadOccurrencesFiles(
            String name, int geoHashPrecision, String startDate, int timeInterval, int nbIntervals
    ) {
        try {
            format.parse(startDate);
        } catch (Exception e) {
        ...
        if (nbIntervals > 0 && timeInterval >= 0) {
            ...
            for (int i = 0; i < nbIntervals; i++) {
                ...
                ret.add(
                    ObisParser.parseOccurrencesJsonObject(FileReader.readJsonObjectFromUrl(
                        "https://api.obis.org/v3/occurrence/grid/" + geoHashPrecision + "?scientificname=" + name
                                + "&startdate=" + startDate + "&enddate=" + endDate
                    ), name)
                );
